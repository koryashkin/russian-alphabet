...
test('opens on the alphabet and exposes all 33 letters', async ({ page }) => {
  ...
  await expect(page.locator('footer')).toContainText('Нашли ошибку или есть идея?')
  await expect(page.getByRole('link', { name: /Напишите в GitHub/ })).toHaveAttribute('href', /github\.com\/koryashkin\/russian-alphabet\/issues/)
})
...
test('every letter opens a complete card', async ({ page }) => {
  ...
  for (const letter of alphabet) {
    ...
    await expect(page.locator('.illustration-prompt')).toContainText('Представь его')
    await page.getByRole('button', { name: 'Показать картинку' }).click()
...
test('letter card has three words and a production image', async ({ page }) => {
  ...
  await expect(page.locator('.word-list button')).toHaveCount(3)
  await expect(page.locator('.illustration-prompt')).toContainText('Послушай слово')
  await expect(page.locator('.letter-illustration')).toHaveCount(0)
  await page.getByRole('button', { name: 'Показать картинку' }).click()
  const image = page.locator('.letter-illustration')
  await expect(image).toBeVisible()
...
test('clicking each word changes the picture and selected state', async ({ page }) => {
  ...
  await expect(page.locator('.illustration-prompt')).toBeVisible()
  await page.getByRole('button', { name: 'Показать картинку' }).click()
