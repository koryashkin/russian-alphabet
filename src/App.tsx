...
function App() {
  ...
  return <main>
    ...
    {view === 'card' && <section className="page-shell card-view"><button className="back-link" onClick={() => setView('alphabet')}>← Вся азбука</button><div className="card-layout"><div className="letter-card"><span className="eyebrow">ЗНАКОМСТВО</span><div className="big-glyph">{selected.uppercase}<small>{selected.lowercase}</small></div>{['М','О','Ё','Ь'].includes(selected.uppercase) ? <div className="word-art generated" role="img" aria-label={`Иллюстрация: ${selected.word}`} style={{backgroundImage:'url(/assets/alphabet-objects.png)',backgroundSize:'200% 200%',backgroundPosition:selected.uppercase==='М'?'0% 0%':selected.uppercase==='О'?'100% 0%':selected.uppercase==='Ё'?'0% 100%':'100% 100%'}} /> : <div className="word-art illustration-pending"><span>Иллюстрация<br/>готовится</span></div>}<div className="word-list">{(examples[selected.uppercase] ?? [selected.accentedWord]).map(word => <button key={word} onClick={() => speak(word)}><Volume2 size={16}/><strong>{word}</strong></button>)}</div><div className="audio-actions"><button onClick={() => speak(`Буква ${selected.uppercase}`)}><Volume2 size={20}/> Слушать букву</button></div>{soundError && <p className="error">Звук не запустился. Нажми ещё раз после касания.</p>}</div><div className="lesson-card"><span className="eyebrow">СЕЙЧАС</span><h2>{selected.uppercase === 'Ь' ? 'Мягкий знак живёт внутри слова' : `Узнаем букву ${selected.uppercase}`}</h2><p>{selected.note}</p><button className="primary full" onClick={() => setView('write')}>Попробовать рукой <ChevronRight size={20}/></button><button className="text-button" onClick={() => speak(selected.uppercase === 'Ь' ? 'У мягкого знака нет своего звука' : selected.word)}>Послушать ещё раз</button></div></div></section>}
